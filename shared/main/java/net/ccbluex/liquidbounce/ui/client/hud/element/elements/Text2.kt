...
class Text2(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    override fun drawElement(): Border? {
        ...
        if (blurValue.get()) {
            ...
            GL11.glPushMatrix()
            BlurUtils.blurArea(floatX * scale - 2F * scale, floatY * scale - 2F * scale, (floatX + fontRenderer.getStringWidth(displayText) + 2F) * scale, (floatY + fontRenderer.fontHeight) * scale, blurStrength.get())
            GL11.glPopMatrix()
        ...
        if (backgroundValue.get()) {
            RenderUtils.drawRect(-2F, -2F, fontRenderer.getStringWidth(displayText) + 2F, fontRenderer.fontHeight + 0F, Color(bgredValue.get(), bggreenValue.get(), bgblueValue.get(), bgalphaValue.get()))
        }

        if (skeetRectValue.get()) {
            drawExhiRect(-4F, if (lineValue.get()) -5F else -4F, fontRenderer.getStringWidth(displayText) + 4F, fontRenderer.fontHeight + 2F)
        }
            ...
            if (suggestion.size > 0) {
                ...
                suggestion.forEachIndexed { index, suggest ->
                    RenderUtils.drawRect(
                        ...
                        fontRenderer.fontHeight * index.toFloat() + 5F, 
                        fontRenderer.getStringWidth(displayText) + 6F + totalLength, 
                        fontRenderer.fontHeight * index.toFloat() + 5F + fontRenderer.fontHeight, 
                        if (index == pointer) Color(90, 90, 90, 120).rgb else Color(0, 0, 0, 120).rgb)
                    fontRenderer.drawStringWithShadow(suggest,
                        (fontRenderer.getStringWidth(displayText) + 4F).toInt(),
                        (fontRenderer.fontHeight * index.toFloat() + 5F).toInt(), -1)
                }
            }
        }
