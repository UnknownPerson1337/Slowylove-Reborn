...
class Inventory :Element() {
    ...
    override fun drawElement(): Border {
        ...
        if (modeValue.get().equals("Default", true) && shadowValue.get()) {
            RenderUtils.drawShadow(8, 28, 150, 65)
        }
        ...
        var itemY: Int = 42
        var airs = 0
        for (i in mc.thePlayer!!.inventory.mainInventory.toString().indices) {
            if (i < 9) continue
            ...
            stack?.let { mc.renderItem.renderItemAndEffectIntoGUI(it, itemX, itemY) }
            stack?.let { mc.renderItem.renderItemOverlays(Fonts.font25, it, itemX, itemY) }
        ...
        if (airs == 27) {
            Fonts.bold35.drawString("Your inventory is empty...", 28, 56, Color(255, 255, 255).rgb)
        }
