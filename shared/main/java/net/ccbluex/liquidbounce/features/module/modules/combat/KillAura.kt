...
//import cc.paimon.utils.GLUtils
...
class KillAura : Module() {
    ...
    private val blockModeValue = ListValue("BlockMode", arrayOf("None", "C07C08", "C07", "Packet", "Fake", "Mouse", "GameSettings", "UseItem","Real" ), "Packet")
    ...
    private val rotations = ListValue("RotationMode", arrayOf("None", "New", "Liquidbounce", "BackTrack","Lockview","Slowylove"), "BackTrack")
                    ...
                    if (target != null) {
                        lastTarget = if (lastTarget == null) {
                            ...
                            if (lastTarget!!.health <= 0) {
                                Minecraft.getMinecraft().connection!!.handleSpawnGlobalEntity(SPacketSpawnGlobalEntity(EntityLightningBolt(mc2.world,
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        if (rotations.get().equals("Slowylove", ignoreCase = true)) {
            if (maxTurnSpeed.get() <= 0F)
                return true
